...
\author{Generated from Verified Comparison Artifacts}
...
MilevTaglianiCN) \cite{MilevTagliani2010a,MilevTagliani2010b}.
Numerical figures are generated from CSV artifacts produced by the
QuantLib\_huatai data pipeline, and the underlying code is independently
validated by 13 passing CTest entries from a dual-version CMake build.
Infrastructure analyses are backed by document and code review artifacts.
We show that 5 of 9 key capabilities are absent in v1.23, and
...
All figures are generated programmatically from CSV data files in
\texttt{QuantLib\_huatai/results/data/}, whose parameters are embedded in
metadata headers. Table~\ref{tab:params} reproduces these metadata values.
The underlying numerical code is independently validated by 13~CTest
entries (Table~\ref{tab:ctest}); note that the CTest suite uses coarser
grids than the publication figures to keep test runtime short.

\begin{table}[htbp]
\centering
\caption{Figure parameters extracted from CSV metadata headers in
  \texttt{QuantLib\_huatai/results/data/}. $N_x$/$N_t$ are the
  \texttt{xGrid}/\texttt{tGrid} values recorded in each file.}
\label{tab:params}
\begin{tabular}{@{}llccccccc@{}}
\toprule
Experiment & Figs & $\sigma$ & $r$ & $K$ & $T$ & $N_x$ & $N_t$ & Barriers \\
\midrule
Truncated call       & 1--2 & 0.001 & 0.05 & 50  & 5/12 & 2801  & 2801 & $U\!=\!70$ \\
Barrier (mod.\ vol)  & 3    & 0.25  & 0.05 & 100 & 0.5  & 4000  & 2000 & $L\!=\!95,\;U\!=\!110$ \\
Barrier (low vol)    & 4    & 0.001 & 0.05 & 100 & 1.0  & 800   & 200  & $L\!=\!95,\;U\!=\!110$ \\
Grid convergence     & 5    & 0.20  & 0.05 & 100 & 1.0  & \multicolumn{2}{c}{varies} & --- \\
Eff.\ diffusion      & 6    & 0.001 & 0.05 & 100 & 1.0  & 200   & ---  & --- \\
M-matrix diagnostic  & 7    & 0.001 & 0.05 & 100 & 1.0  & 200   & ---  & --- \\
Benchmark            & 8    & 0.20  & 0.05 & 100 & 1.0  & \multicolumn{2}{c}{varies} & --- \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[htbp]
\centering
\caption{CTest validation summary (13/13 pass). These tests use
  coarser grids than the publication figures but verify the same code paths.}
\label{tab:ctest}
\begin{tabular}{@{}lll@{}}
\toprule
CTest Name & Result & Grid \\
\midrule
\texttt{oscillation\_v123}          & 5 neg nodes, min $V = -6.150$  & $N_x\!=\!200$ \\
\texttt{positivity\_huatai}         & EF/MT: 0 neg nodes             & $N_x\!=\!200$ \\
\texttt{discrete\_barrier\_huatai}  & SC: 58 neg; EF/MT: 0           & $N_x\!=\!200$ \\
\texttt{mmatrix\_diagnostic\_huatai}& SC: 198 neg off-diags          & $N_x\!=\!200$ \\
\bottomrule
\end{tabular}
\end{table}
...
$K = 50$, $U = 70$, $T = 5/12$, and $N_x = 2801$. At this resolution,
the StandardCentral scheme produces 139~negative interior nodes with
minimum $V = -4.765$, confirming the M-matrix violation predicted by
...
\caption{Spurious oscillations in Crank--Nicolson at extreme low
  volatility ($\sigma=0.001$, $N_x=2801$). The StandardCentral scheme
  produces 139 negative option values near the truncation point $U=70$.
  Data: \texttt{truncated\_call\_StandardCentral.csv}.}
...
spikes, ExponentialFitting maintains strict positivity (0~negative nodes).
MilevTaglianiCN has one node at $-7.8 \times 10^{-123}$---negative only
at machine precision, effectively zero. The fine-grid reference
(ExponentialFitting at $N_x = 22{,}408$) confirms convergence.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/fig2_three_scheme_truncated.pdf}
\caption{Three-scheme comparison on truncated call ($N_x=2801$).
  Left: full view showing StandardCentral oscillations vs.\ smooth
  nonstandard solutions. Right: zoom near truncation point $U=70$.
  Data: \texttt{truncated\_call\_*.csv}.}
...
schemes produce positive prices that agree closely (0.234 at $S \approx K = 100$
with $N_x = 4000$), confirming that the nonstandard schemes reduce to
...
\caption{Discrete double barrier at moderate volatility ($\sigma=0.25$,
  $K=100$, $L=95$, $U=110$, $N_x=4000$, 5~monitoring dates). All three
  schemes agree.
  Data: \texttt{barrier\_moderate\_vol\_*.csv}.}
\label{fig:barrier_mod}
\end{figure}

\paragraph{Low volatility ($\sigma = 0.001$).}
Figure~\ref{fig:barrier_low} reveals the critical failure mode:
at $N_x = 800$, StandardCentral produces 52~negative nodes (minimum
$V = -1.968$) while both nonstandard schemes maintain strict positivity
with 0~negative nodes.
...
\caption{Discrete double barrier at extreme low volatility
  ($\sigma=0.001$, $N_x=800$). StandardCentral: 52 negative nodes.
  ExponentialFitting and MilevTaglianiCN: 0 negative nodes.
  Data: \texttt{barrier\_low\_vol\_*.csv}.}
...
$a_{i,i-1}$ and $a_{i,i+1}$ of the operator matrix at $N_x = 200$.
StandardCentral has 198~negative lower off-diagonal entries (minimum
$= -3.578$). ExponentialFitting has negatives only at machine precision
($\sim\!4 \times 10^{-16}$), and MilevTaglianiCN has strictly positive
off-diagonals.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/fig7_mmatrix.pdf}
\caption{M-matrix off-diagonal signs at $\sigma=0.001$, $N_x=200$.
  Negative entries (below the red line) cause spurious oscillations.
  StandardCentral: 198 violations (min $= -3.578$). MilevTaglianiCN: 0.
  Data: \texttt{mmatrix\_offdiag\_*.csv}.}
\label{fig:mmatrix}
\end{figure}
